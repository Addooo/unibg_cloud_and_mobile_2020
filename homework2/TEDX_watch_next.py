...
from pyspark.sql.functions import col, collect_list, array_join #alcune funzioni particolari di pyspark
...
tedx_dataset_watch = tedx_dataset_agg.join(watch_dataset_agg, tedx_dataset_agg.idx == watch_dataset_agg.idx_ref , "left").select(col("idx").alias("_id"), col("*")).drop("idx")
...
tedx_dataset_dynamic_frame = DynamicFrame.fromDF(tedx_dataset_watch, glueContext, "nested")#converto il db glue in un spark (?)
